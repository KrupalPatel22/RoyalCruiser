...
class _BoardingPointWidgetState extends State<BoardingPointWidget> {
  ...
  void onTapData({required int index}) {
    ...
    NavigatorConstants.SelectedBoardingPointNameForPage = widget.boardingPointList[index].PickUpName;
    if (tripType == "0" || tripType == "1") {
      setState(() {
        NavigatorConstants.SelectedBoardingPointOnwordID = widget.boardingPointList[index].PickUpID;
        NavigatorConstants.SelectedBoardingPointOnwordName = widget.boardingPointList[index].PickUpName;
        NavigatorConstants.SelectedBoardingPointOnwordTime = widget.boardingPointList[index].PickUpTime;
      });
      widget.onNext();
    } else if (tripType == "2") {
      setState(() {
        NavigatorConstants.SelectedBoardingPointReturnID = widget.boardingPointList[index].PickUpID;
        NavigatorConstants.SelectedBoardingPointReturnName = widget.boardingPointList[index].PickUpName;
        NavigatorConstants.SelectedBoardingPointReturnTime = widget.boardingPointList[index].PickUpTime;
      });
      widget.onNext();
    }
  }
